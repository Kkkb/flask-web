...
class APITestCase(unittest.TestCase):
	...
	def test_no_auth(self):
		...
		# the API is designed to accept anonymous users. Anybody can
		# request posts, without providing authentication. So this unit
		# test should ensure that the status code returned by the GET
		# request to /posts is 200, not 401. See https://github.com/miguelgrinberg/flasky/issues/83
	...
	def test_bad_auth(self):
		...
		# fixed by change 'basic' to 'basic ' in get_api_headers
	...
	def test_users(self):
		...
		r = Role.query.filter_by(name='User').first()
		self.assertIsNotNone(r)
		u1 = User(email='gakki@love.me', username='gakki',
				  password='cat', confirmed=True, role=r)
		u2 = User(email='k@love.me', username='k',
		...
		self.assertEqual(response.status_code, 200)
		json_response = json.loads(response.get_data(as_text=True))
		self.assertEqual(json_response['username'], 'gakki')
		response = self.client.get(
			'/api/v1/users/{}'.format(u2.id),
			headers=self.get_api_headers('k@love.me', 'dog'))
		self.assertEqual(response.status_code, 200)
		json_response = json.loads(response.get_data(as_text=True))
		self.assertEqual(json_response['username'], 'k')
	...
	def test_comments(self):
		...
		self.assertEqual(json_response['body'],
						 'Good [post](https://github.com/kkkb)!')
		...
		self.assertEqual(json_response['body'],
						 'Good [post](http://github.com/kkkb)!')
